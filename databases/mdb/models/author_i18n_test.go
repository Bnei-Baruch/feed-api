...
func testAuthorI18nsDelete(t *testing.T) {
	...
	count, err := AuthorI18ns().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
...
func testAuthorI18nsQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := AuthorI18ns().DeleteAll(ctx, tx); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testAuthorI18nsExists(t *testing.T) {
	...
	e, err := AuthorI18nExists(ctx, tx, o.AuthorID, o.Language)
	if err != nil {
...
func testAuthorI18nsFind(t *testing.T) {
	...
	authorI18nFound, err := FindAuthorI18n(ctx, tx, o.AuthorID, o.Language)
	if err != nil {
		t.Error(err)
	}
...
func testAuthorI18nsBind(t *testing.T) {
	...
	if err = AuthorI18ns().Bind(ctx, tx, o); err != nil {
		t.Error(err)
	}
}
...
func testAuthorI18nsOne(t *testing.T) {
	...
	if x, err := AuthorI18ns().One(ctx, tx); err != nil {
		t.Error(err)
	} else if x == nil {
...
func testAuthorI18nsAll(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = authorI18nOne.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Error(err)
	}
	if err = authorI18nTwo.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Error(err)
	}

	slice, err := AuthorI18ns().All(ctx, tx)
	if err != nil {
		t.Error(err)
	}
...
func testAuthorI18nsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(authorI18nColumnsWithoutDefault...)); err != nil {
		t.Error(err)
	}
...
func testAuthorI18nToOneAuthorUsingAuthor(t *testing.T) {
	...
	if err = local.L.LoadAuthor(ctx, tx, false, (*[]*AuthorI18n)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAuthorI18nToOneSetOpAuthorUsingAuthor(t *testing.T) {
	...
	for i, x := range []*Author{&b, &c} {
		...
		if exists, err := AuthorI18nExists(ctx, tx, a.AuthorID, a.Language); err != nil {
			t.Fatal(err)
		} else if !exists {
