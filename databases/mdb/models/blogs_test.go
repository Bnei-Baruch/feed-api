...
func testBlogToManyBlogPosts(t *testing.T) {
	...
	check, err := a.BlogPosts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := BlogSlice{&a}
	if err = a.L.LoadBlogPosts(ctx, tx, false, (*[]*Blog)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.BlogPosts = nil
	if err = a.L.LoadBlogPosts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.BlogPosts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testBlogToManyAddOpBlogPosts(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddBlogPosts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.BlogPosts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
