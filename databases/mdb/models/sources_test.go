...
func testSourceToManyAuthors(t *testing.T) {
	...
	slice := SourceSlice{&a}
	if err = a.L.LoadAuthors(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testSourceToManyContentUnits(t *testing.T) {
	...
	slice := SourceSlice{&a}
	if err = a.L.LoadContentUnits(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testSourceToManySourceI18ns(t *testing.T) {
	...
	check, err := a.SourceI18ns().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := SourceSlice{&a}
	if err = a.L.LoadSourceI18ns(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.SourceI18ns = nil
	if err = a.L.LoadSourceI18ns(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SourceI18ns); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testSourceToManyParentSources(t *testing.T) {
	...
	check, err := a.ParentSources().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := SourceSlice{&a}
	if err = a.L.LoadParentSources(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadParentSources(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ParentSources); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testSourceToManyAddOpSourceI18ns(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddSourceI18ns(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.SourceI18ns().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testSourceToManyAddOpParentSources(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddParentSources(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.ParentSources().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testSourceToManySetOpParentSources(t *testing.T) {
	...
	err = a.SetParentSources(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetParentSources(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testSourceToManyRemoveOpParentSources(t *testing.T) {
	...
	err = a.AddParentSources(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveParentSources(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
func testSourceToOneSourceUsingParent(t *testing.T) {
	...
	if err = local.L.LoadParent(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testSourceToOneSourceTypeUsingType(t *testing.T) {
	...
	slice := SourceSlice{&local}
	if err = local.L.LoadType(ctx, tx, false, (*[]*Source)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Type == nil {
		t.Error("struct should have been eager loaded")
